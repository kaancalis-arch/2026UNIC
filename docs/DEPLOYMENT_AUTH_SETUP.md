...
Webhook URL'sini `VITE_N8N_WEBHOOK_URL` adıyla tanımlamayın; `VITE_*` değerleri istemciye açılır. OpenAI çağrıları kimlik doğrulamalı `ai-advisor` Supabase Edge Function üzerinden yapılır ve OpenAI secret'ı frontend'e aktarılmaz.

`vercel.json`; parola sıfırlama ile güvenli belge/rapor paylaşım yollarını SPA giriş dosyası `/index.html` üzerine rewrite eder. Catch-all rewrite yoktur; `/api/*` ve `/assets/*` bu kurallardan etkilenmez.
...
- `OPENAI_API_KEY`, yalnız `ai-advisor` Supabase Edge Function tarafından kullanılan server-side secret'tır. Vercel frontend/build ortamına, `VITE_OPENAI_*` veya başka bir `VITE_*` değişkenine, yerel `.env` dosyalarına ya da GitHub'a eklenemez.
- `AI_PROVIDER` yalnız `openai` değerini kabul eder. `OPENAI_MODEL` sunucu tarafından seçilir; kullanıcı arayüzünde model seçimi veya API anahtarı alanı yoktur.
...
`SUPABASE_SERVICE_ROLE_KEY`, `PROJECT_SUPABASE_SERVICE_ROLE_KEY` ve `OPENAI_API_KEY` sunucu sırlarıdır.
...
- Service role key'ler yalnız Supabase Edge Function secret ortamında ve gerçekten server-side çalışan güvenilir servislerde kullanılmalıdır. `OPENAI_API_KEY` ise yalnız Supabase Edge Function secret ortamında tutulmalıdır.
...
5. Hiçbir service role key'in veya `OPENAI_API_KEY` değerinin Vercel frontend ortamında, GitHub'da, yerel `.env` dosyalarında veya `VITE_*` değişkenlerinde bulunmadığını doğrulayın.
